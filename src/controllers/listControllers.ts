...
async function listTeachersBySubjectId(req: Request, res: Response, next: NextFunction) {
  try {
    const { subjectId } = req.params;
    if(!Number(subjectId)){
      throw new BadRequestError("id deve ser um numero")
    }
    const result = await listService.getAllTeachersBySubjectId(Number(subjectId));
    res.status(200).send(result);
  } catch (error) {
    if(error.name === 'NotFoundError') {
      return res.status(404).send(error.message)
